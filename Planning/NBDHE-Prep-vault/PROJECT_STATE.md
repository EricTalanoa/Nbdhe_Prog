...
b22 — rotated depth across Care Planning, Perio Management, and Local Anesthesia (Pharmacology):
q-plan-0025 (Care Planning/treatment strategies-diagnosis — prioritizing competing dental hygiene
diagnoses via the human needs model when an acute abscess, plaque etiology, and a patient's
stated learning goal compete), q-perio-0024 (Perio Management/prescribed therapy-chemotherapeutic
agents — triclosan/copolymer dentifrice's substantivity mechanism), q-perio-0025 (Perio
...
**Batches 7-22 (6 questions + 1 flashcard, then 8, then 8, then 8, then 6 questions + 2 flashcards,
...
1. Next chunk: **7b-bank-depth** (ongoing) — keep deepening the bank, one focused batch/run.
   Batch 22 rotated depth across Care Planning (treatment strategies-diagnosis/prioritizing
   competing dental hygiene diagnoses via the human needs model), Perio Management (prescribed
   therapy-chemotherapeutic agents/triclosan-copolymer substantivity; reassessment and
   evaluation/bleeding-on-probing negative predictive value), and Local Anesthesia
   (Pharmacology/protein binding as the determinant of duration of action) — every subdomain
   bank-wide already has ≥2 items, so future batches keep pushing the thinnest ones to the next
   pass. A Community Health testlet is still the next *new* content type, but that needs its own
   infra chunk first (`scripts/import-questions.mjs` has no testlet parser/upsert yet, and no UI
   wires a testlet's scenario into the practice-loop stimulus the way `PatientBox` does for cases —
   see AUTOPILOT.md's open item). Otherwise, keep rotating depth through the highest-yield clinical
   areas (Local Anesthesia, Care Planning, Perio Management) and keep authoring dedicated
   flashcards (`fc-*.md`) alongside questions. Also: apply batches 7-22 content live (see #2).
